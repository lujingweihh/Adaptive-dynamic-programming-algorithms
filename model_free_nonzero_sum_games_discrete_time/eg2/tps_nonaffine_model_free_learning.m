function tps_nonaffine_model_free_learning
...
rho_a1_initial = 0.001;
rho_a2_initial = 0.001;
...
rho_a1_final = 1e-7; 
rho_a2_final = 1e-7; 
rho_a1_decay = (rho_a1_initial-rho_a1_final)/(0.6*N/decay_interval);
rho_a2_decay = (rho_a2_initial-rho_a2_final)/(0.6*N/decay_interval);
...
for k = 1:N
    ...
    if actor_update_flag == 1
        wa1 = actor_weights(s,s_next,wc1,wa1,rho_a1,Ra11,Ga1);
        wa2 = actor_weights(s,s_next,wc2,wa2,rho_a2,Ra22,Ga2);
    end
...
%--------------------------- system dynamics ------------------------------
function x_next = system_dynamics(x,u1,u2)

T = 0.1;

fx = [       x(1) + T*x(2);...
       (1-T)*x(2) - T*0.5*x(1) + T*0.25*x(2)*(cos(2*x(1))+2)^2 + T*0.25*x(2)*(sin(4*x(1)^2)+2)^2  ];

gx1 = T*[ 0; 2 ];
gx2 = T*[ 0; 2 ];
gx3 = T*[ 0; 0.1 ];

x_next = fx + gx1*(u1+tanh(u1)) + gx2*(u2) + gx3*u1*u2;
