% This demo checks the feasibility of the intetral reinforcement 
% learning (IRL) algorithm 
%
% By J. Lu
% Date: Nov. 23, 2020 
...
P = zeros(4,4);
uu = 0; % record the control signal u
...
% initial condition
x0 = [0 0.1 0 0 0]; % the last one is the integral of x'Qx + u'Ru

% a stable control policy K
...
Fsamples = 120;
T = 0.05; % sample time 
SS = 0.001; % step size for the simulation
...
for k=1:Fsamples
    ...
    waitbar(k/Fsamples,h,['Training controller...',num2str(k/Fsamples*100),'%']);
end
close(h);
